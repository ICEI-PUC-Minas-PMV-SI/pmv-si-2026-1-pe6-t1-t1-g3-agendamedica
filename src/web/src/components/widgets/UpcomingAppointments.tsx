    ...
    <div className="appt-row">
      ...
      <div className="appt-body">
        <div className="appt-doctor">
          {ap.doctor}
          <span className={`chip ${STATUS_CLASS[ap.status]}`}>{STATUS_LABEL[ap.status]}</span>
        </div>
        <div className="appt-specialty">
          <span className="inline-ic"><Ic.stethoscope size={13} />{ap.specialty}</span>
      ...
      <div className="appt-actions">
        {ap.mode === 'tele' && ap.status === 'PENDING' && (
          <button className="btn btn-ghost btn-sm"><Ic.video size={14} /> Entrar</button>
        )}
        {ap.status !== 'CANCELLED' && (
          <>
    ...
    <section className="card">
      <div className="card-head">
        ...
        <button className="card-action">
          Ver todas <Ic.arrow size={12} style={{ verticalAlign: 'middle', marginLeft: 2 }} />
        </button>
      </div>
      ...
      {state === 'loaded' && (
        <div>{appointments.map(ap => <AppointmentRow key={ap.id} ap={ap} />)}</div>
      )}
    </section>
  )
}
